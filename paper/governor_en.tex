...
\texttt{verify\_verlet\_schurcohn.py}: the symbolic identity, $302\,400$ cells
...
benchmark and its paired analysis (\texttt{benchmark\_v3.py},
\texttt{benchmark\_report\_v2.py}), the v4 deconfounding campaign
(\texttt{PREREG\_V4.md}, \texttt{benchmark\_v4.py},
\texttt{results\_benchmark\_v4/report\_v4.json}), the accumulator kill-gate
(\texttt{mhbp/tasks/reasoner\_g0/n4\_g1.py}), the KdV zoo (\texttt{benchmark\_zoo.py}), the
non-locality A/B (\texttt{benchmark\_elliptic.py}), and the mechanism-null studies
(\texttt{\_alpha\_scan.py}, \texttt{\_pde\_study.py}, \texttt{\_twin\_pilot.py},
\texttt{\_gates\_flowroute.py}). A regression test verifies that the field is
actually \emph{wired in} ---ablating it changes the forward pass and the
compute policy, though not accuracy--- (\texttt{\_audit\_hbp.py}), a check of
plumbing rather than of the functional load this paper's null denies; and physical verifications of the
2D solver in \texttt{\_check\_flow2d.py}. The physical parameters are anchored to
...
\texttt{certify\_lmi.py}, \texttt{certify\_lmi\_ckpt.py} and
\texttt{certify\_lmi\_iss.py}.
